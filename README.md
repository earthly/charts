...
| `hub.db.sqlapiSslRootCert` | PEM bundle of the CA that signs the SQL API server certificate. The chart puts it in a ConfigMap and the Hub vends its contents with the connection string, so `lunar sql connection-string` can fill in `sslrootcert=` itself. Requires Hub 3.20.0+. See the notes below. | `""` |
