...
| `grafana.dataVolume.type` / `.emptyDir` | Storage mounted at `/var/lib/grafana` (Grafana's SQLite database, installed plugins, sessions). `emptyDir` preserves it across container restarts within one pod; a replacement pod starts fresh and is re-provisioned by the sidecar. `none` mounts nothing, for installations that already mount the path through `grafana.volumeMounts`. `.emptyDir` accepts the Kubernetes `emptyDir` options. The generated volume is named `grafana-data` (reserved in `grafana.volumes`) | `emptyDir` / `{}` |
